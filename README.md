...
## Dataset structure 
The dataset consists of six NetCDF files providing estimates of:

1. the global warming level (GWL) at which present-day extreme sea-level (ESL) events become annual events,
2. the timing at which these transitions occur, and
3. ESL return levels associated with different return periods and GWLs.

Two spatial domains are included:
larger set (7,283 locations), based on the hydrodynamic-model-derived present-day ESL datasets of Vousdoukas et al. (2018) and Kirezci et al. (2020);
smaller set (179 locations), which additionally incorporates the tide-gauge-based present-day ESL dataset of Rasmussen et al. (2018).

### Global warming level datasets
#### Files: 
- GWL_VS_FC_larger_set.nc
- GWL_VS_FC_smaller_set.nc
#### Variables: 
- longitude (degrees_east): longitude of coastal location
- latitude (degrees_north): latitude of coastal location
- location_id (unitless): original grid-point identifier to link each coastal location to the source datasets
- GWL (degree Celsius): global warming level at which a present-day return period becomes annual

Values represent global warming levels, encoded as categories corresponding to 1.5°C, 2°C, 2.5°C, 3°C, 4°C, 5°C, and >5°C. Locations for which the transition does not occur are assigned the missing-value flag -9999.
#### Dimensions: 
- location: coastal locations (7,283 for the larger set; 179 for the smaller set)
- return_period: present-day ESL return periods of 10, 20, 50, 100, and 200 years
- method: Voting System (VS) and Full Convolution (FC)
- uncertainty: 5th percentile (q0.05), median (q0.5), and 95th percentile (q0.95)
#### Global attributes:
- title
- institution
- source
...
### Timing datasets
#### Files
- Timing_FC_larger_set.nc
- Timing_FC_smaller_set.nc
#### Variables
- longitude (degrees_east): longitude of coastal location
- latitude (degrees_north): latitude of coastal location
- location_id (unitless): original grid-point identifier to link each coastal location to the source datasets
- first_decade (year): the first decade in which a given ESL RP becomes annual

Values represent the first decade (2020–2100, in 10-year increments) at which the specified return period becomes annual under the Full Convolution method. Locations for which the transition does not occur by 2100 are assigned the missing-value flag -9999.
#### Dimensions
- location: coastal locations (7,283 for the larger set; 179 for the smaller set)
- uncertainty: 5th percentile (q0.05), median (q0.5), and 95th percentile (q0.95)
- GWL: global warming levels (1.5°C, 2°C, 2.5°C, 3°C, 4°C, and 5°C)
- return_period: present-day ESL return periods of 10, 20, 50, 100, and 200 years
#### Global attributes:
- title
- institution
...
#### Files
- Return_levels_larger_set.nc
- Return_levels_smaller_set.nc
#### Variables
- longitude (degrees_east): longitude of coastal location
- latitude (degrees_north): latitude of coastal location
- location_id (unitless): original grid-point identifier to link each coastal location to the source datasets
- return_level_value (cm): ESL associated with a given return period and global warming level

Values represent ESL return levels for each return period, uncertainty quantile, and global warming level.
#### Dimensions
- location: coastal locations (7,283 for the larger set; 179 for the smaller set)
- return_period: present-day ESL return periods of 10, 20, 50, 100, and 200 years
- uncertainty: 5th percentile (q0.05), median (q0.5), and 95th percentile (q0.95)
